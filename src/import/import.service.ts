 ...
 * Col 17: Latitud y Longitud (Una cámara)
 * Col 18-19: Dos cámaras lat/lng
 * Col 20-22: Tres cámaras
 * ... (patrón continúa hasta col 152)
 * Col 154: Mes
 * Col 155: Fecha de registro
 */
...
const SECTION_START: Record<number, number> = {
...
export class ImportService {
  ...
  private async processCamaras(
    ...
    const startCol = SECTION_START[numCams];
    if (!startCol) return;
  ...
  async importSheet1(buffer: Buffer, dryRun = false): Promise<ImportResult> {
    ...
    try {
      ...
      for (let i = 1; i < rows.length; i++) {
        ...
        try {
          ...
          result.errores.push(`Fila ${i + 1}: ${e.message}`);
        }
      }
    } catch (e) {
      result.errores.push(`Error procesando archivo: ${e.message}`);
    }
  ...
  async importSheet2(buffer: Buffer, dryRun = false): Promise<ImportResult> {
    const result = this.emptyResult();
    const wb = XLSX.read(buffer, { type: 'buffer' });

    try {
      // Se asume que el archivo subido tiene solo esta hoja (SheetNames[0])
      // Si el usuario sube el Excel completo, cambia a SheetNames[1]
      const ws = wb.Sheets[wb.SheetNames[0]];
      const rows: any[][] = XLSX.utils.sheet_to_json(ws, { header: 1, defval: null });
      let sectorActual = '';
  ...
  async importSheet3(buffer: Buffer, dryRun = false): Promise<ImportResult> {
    const result = this.emptyResult();
    const wb = XLSX.read(buffer, { type: 'buffer' });

    try {
      // Se asume que el archivo subido tiene solo esta hoja (SheetNames[0])
      // Si el usuario sube el Excel completo, cambia a SheetNames[2]
      const ws = wb.Sheets[wb.SheetNames[0]];
      const rows: any[][] = XLSX.utils.sheet_to_json(ws, { header: 1, defval: null });
      let sectorActual = '';
  ...
  // ─── Legacy: las 3 hojas en un solo Excel ────────────────────────────────
